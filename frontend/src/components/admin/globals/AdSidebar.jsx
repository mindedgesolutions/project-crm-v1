      ...
      >
        <div className="flex justify-center items-center my-4 p-4">
          <Link to={`/admin/dashboard`}>
            <h3 className="text-5xl font-bold tracking-widest text-primary">
              CRM
            </h3>
          </Link>
        </div>
        <div className="flex flex-col h-full">
          <ul className="whitespace-pre px-2.5 text-[0.9rem] py-5 flex flex-col gap-1  font-medium overflow-x-hidden scrollbar-thin scrollbar-track-white scrollbar-thumb-slate-100 md:h-[68%] h-[70%]">
            <li>
              <NavLink
                to={`/admin/dashboard`}
                className={`w-full flex flex-row justify-start items-center p-2 gap-2 rounded-lg hover:bg-card ${
                  pathname === `/admin/dashboard` ? "bg-card" : null
                }`}
              >
                <Home size={18} className="text-muted-foreground" />
                <p className="text-muted-foreground font-medium tracking-wider">
                  Dashboard
                </p>
              </NavLink>
            </li>
